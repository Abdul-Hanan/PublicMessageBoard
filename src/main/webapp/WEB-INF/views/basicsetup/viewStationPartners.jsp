...
<head>
    <title>Station Partner :: Raseel</title>
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-12">
                                <div class="page-header-title">
                                    <h5 class="m-b-10">Station Partners</h5>
                                </div>
                                <ul class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="#"><i class="feather icon-home"></i></a></li>
                                    <li class="breadcrumb-item"><a href="#!">Station Partners</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-sm-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5>Station Partners</h5>
                                        <div class="card-header-right">
                                            <sec:authorize access="hasAuthority('create-stationpartner')">
                                                <button type="button" onclick="location = 'stationpartner'" class="btn btn-primary" title="" data-toggle="tooltip" data-original-title="Create a New Station Partnet">New Partner</button>
                                            </sec:authorize>
                                        </div>
                                    </div>
                                    <div class="card-block">
                                        <div class="table-responsive">
                                            <table id="key-act-button" class="display table nowrap table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                <c:forEach varStatus="loop" var="stationPartner" items="${stationPartners}">
                                                    <tr>
                                                        <td>${loop.index+1}</td>
                                                        <td>${stationPartner.companyName}</td>
                                                        <td>${stationPartner.companyAddress}</td>
                                                        <td>${stationPartner.contactFirstName} ${stationPartner.contactLastName}</td>
                                                        <td>${stationPartner.contactEmail}</td>
                                                        <td>${stationPartner.contactPhone}</td>
                                                        <td>${stationPartner.user.username}</td>
                                                        <td>${stationPartner.user.enabled eq true ? 'Yes':'No'}</td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('edit-stationpartner')">
                                                                <a href="${contextPath}/stationpartner?id=${stationPartner.id}" class="text-left" data-cont="forms"><i class="feather feather icon-edit"></i></a>
                                                            </sec:authorize>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
