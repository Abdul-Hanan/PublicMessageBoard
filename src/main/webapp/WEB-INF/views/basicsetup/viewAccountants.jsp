                        ...
                        <div class="row align-items-center">
                            <div class="col-md-12">
                                ...
                                <ul class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="#!">Accountants</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-sm-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5>Manage Accountants</h5>
                                        <div class="card-header-right">
                                            <sec:authorize access="hasAuthority('create-accountant')">
                                                <button type="button" onclick="location = 'accountant'" class="btn btn-primary btn-icon" title="" data-toggle="tooltip" data-original-title="Create a New Accountant"><i class="feather icon-plus"></i></button>
                                            </sec:authorize>
                                        </div>
                                    </div>
                                    <div class="card-block">
                                        <div class="table-responsive">
                                            <table id="key-act-button" class="display table nowrap table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                <c:forEach varStatus="loop" var="accountant" items="${accountants}">
                                                    <tr>
                                                        <td>${loop.index+1}</td>
                                                        <td>${accountant.contactFirstName} ${accountant.contactLastName}</td>
                                                        <td>${accountant.contactEmail}</td>
                                                        <td>${accountant.contactPhone}</td>
                                                        <td>${accountant.user.username}</td>
                                                        <td>${accountant.user.enabled eq true ? 'Yes':'No'}</td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('edit-accountant')">
                                                                <a href="${contextPath}/accountant?id=${accountant.id}" class="text-left" data-cont="forms"><i class="feather feather icon-edit"></i></a>
                                                            </sec:authorize>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
