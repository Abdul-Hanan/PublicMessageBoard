                        ...
                        <div class="row align-items-center">
                            <div class="col-md-12">
                                <div class="page-header-title">
                                    <h5 class="m-b-10">Raseel Receivers</h5>
                                </div>
                                <ul class="breadcrumb">
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <div class="card-header-right">
                                            <sec:authorize access="hasAuthority('create-receiver')">
                                                <button type="button" onclick="location = 'receiver'" class="btn btn-primary btn-icon" title="" data-toggle="tooltip" data-original-title="Create a New Receiver"><i class="feather icon-plus"></i></button>
                                            </sec:authorize>
                                        </div>
                                    </div>
                                            ...
                                            <table id="key-act-button" class="display table nowrap table-hover" style="width:100%">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>Area</th>
                                                        <th>Full Address</th>
                                                        <th>Full Arabic Address</th>
                                                        <th></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                <c:forEach varStatus="loop" var="receiver" items="${receivers}">
                                                    <tr>
                                                        <td>${loop.index+1}</td>
                                                        <td>${receiver.fullName}</td>
                                                        <td>${receiver.email}</td>
                                                        <td>${receiver.contactNumber}</td>
                                                        <td>${receiver.secondaryContactNumber}</td>
                                                        <td>${receiver.city.name}</td>
                                                        <td>${receiver.area.name}</td>
                                                        <td>${receiver.fullAddress}</td>
                                                        <td>${receiver.fullArabicAddress}</td>
                                                        <td>
                                                            <%-- <sec:authorize access="hasAuthority('edit-admin')">
                                                                <a href="${contextPath}/admin?id=${admin.id}" class="text-left" data-cont="forms"><i class="feather feather icon-edit"></i></a>
                                                            </sec:authorize> --%>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
