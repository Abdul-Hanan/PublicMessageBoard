...
<head>
    <title>Raseel Customers :: Raseel</title>
...
<div class="pcoded-main-container">
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-12">
                                <div class="page-header-title">
                                    <h5 class="m-b-10">Raseel Customers</h5>
                                </div>
                                <ul class="breadcrumb">
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <div class="card-header-right">
                                            <sec:authorize access="hasAuthority('create-customer')">
                                                <button type="button" onclick="location = 'customer'" class="btn btn-primary btn-icon" title="" data-toggle="tooltip" data-original-title="Create a New Customer">
 ...
 <div style="float: right;">
										  ...
										  <a onclick="disableCustomers();" style="color: black"	class="btn btn-sm btn-rounded btn-outline-primary md-trigger"
                                       ...
                                       <div class="table-responsive">
                                                ...
                                                <tbody>
                                                <c:forEach varStatus="loop" var="customer" items="${customers}">
                                                
                                                    <tr style="background-color: ${customer.user.enabled eq false ? '#f1f1f1':'#ffffff'}">
                                                        <td><input type="checkbox" name="selectedCustomers[]" class="selectedCustomers" value="${customer.id}" class="selects"></td>
                                              
                                                        <td>${loop.index+1}</td>
                                                        <td>
                                                            <img class="media-object img-radius" height="30px" width="30px" src="${contextPath}/getCustomerImage?id=${customer.id}" alt="Generic placeholder image ">
                                                        </td>
                                                        <td>${customer.customerName}</td>
                                                        <td>${customer.customerArabicName}</td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('view-customeruser')">

                                                                <a href="${contextPath}/customerUsers?customerId=${customer.id}"
                                                                   class="btn btn-sm btn-rounded btn-outline-primary" data-cont="forms">
                                                                    <span class="badge badge-primary">${fn:length(customer.customerUsers)}</span>
                                                                    Users
                                                                </a>
                                                            </sec:authorize>
                                                        </td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('view-customeroffice')">
                                                                <a href="${contextPath}/customerOffices?customerId=${customer.id}" class="btn btn-sm btn-rounded btn-outline-primary" data-cont="forms">
                                                                    <span class="badge badge-primary">${fn:length(customer.customerOffices)}</span>
                                                                    Branches</a>
                                                            </sec:authorize>
                                                        </td>
                                                        <td>${customer.customerAddress}</td>
                                                        <td>${customer.customerArea.name}</td>
                                                        <td>${customer.contactFirstName} ${customer.contactLastName}</td>
                                                        <td>${customer.contactEmail}</td>
                                                        <td>${customer.contactPhone}</td>
                                                        <td>${customer.user.username}</td>
                                                        <td><span>${customer.user.enabled eq true ? 'Yes':'No'}</span></td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('edit-customer')">
                                                                <a href="${contextPath}/customer?id=${customer.id}" class="text-left" data-cont="forms"><i class="feather feather icon-edit"></i></a>
                                                            </sec:authorize>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
...
function disableCustomers(){
    ...
    if(customerNos=="0"){
        ...
        // Run ajax to diable here
    	 $.ajax({
             url: '${contextPath}/disableCustomers',
             data: {customernos : customerNos},
             type: 'GET',
             cache: false,
