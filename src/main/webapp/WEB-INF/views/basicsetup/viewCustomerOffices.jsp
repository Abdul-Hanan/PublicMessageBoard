...
<head>
    <title>${customer.customerName} Branches :: Raseel</title>
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <div class="card-header-right">
                                            <sec:authorize access="hasAuthority('create-customeroffice')">
                                                <button type="button" onclick="location = 'customerOffice?customerId=${param.customerId}'" class="btn btn-primary btn-icon" title="" data-toggle="tooltip" data-original-title="Create a New Branch">
                                    ...
                                    <div class="card-block">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-4">
                                                <form method="get" action="${contextPath}/customerOffices?customerId=${param.customerId}">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-12">
                                                            <div class="form-group" style="display: none;">
                                                                ...
                                                                <select name="locationTypeId" class="form-control js-example-basic-single" id="locationType">
                                                                    <option disabled>Select Location Type</option>
                                                                    <c:forEach var="locationType" items="${locationTypes}">
                                                                        <option value="${locationType.id}" ${param.locationTypeId eq locationType.id ?'selected':''}>${locationType.name}</option>
                                                                    </c:forEach>
                                                                </select>
                                                            </div>
                                                        </div>
                                        ...
                                        <div class="table-responsive">
                                            <table id="key-act-button" class="display table nowrap table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                <c:forEach varStatus="loop" var="customerOffice" items="${customerOffices}">
                                                    <tr>
                                                        <td>${loop.index+1}</td>
                                                        <td>${customerOffice.geoFence.name} </td>
                                                        <td>${customerOffice.geoFence.address}</td>
                                                      	<td>
                                                                <a href="${contextPath}/customerBranchUsers?branchId=${customerOffice.id}" class="btn btn-sm btn-rounded btn-outline-primary" data-cont="forms">
                                                                    <span class="badge badge-primary">${branchUsers[customerOffice.id]}</span>
                                                                     Users</a>
                                                      	</td>
                                                       	<td>${customerOffice.zone.name} </td>        
                                                       	<td>${customerOffice.zone.zoneName} </td>                                                         
                                                        <td>${customerOffice.geoFence.city.name}</td>
                                                        <td>${customerOffice.geoFence.country.name}</td>
                                                        <td>${customerOffice.geoFence.locationType.name}</td>
                                                        <td>${customerOffice.geoFence.enabled eq true ? 'Yes':'No'}</td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('edit-customeroffice')">
                                                                <a href="${contextPath}/customerOffice?id=${customerOffice.id}&customerId=${param.customerId}" class="text-left" data-cont="forms"><i class="feather feather icon-edit"></i></a>
                                                            </sec:authorize>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
    ...
    function initMap() {
        ...
        <c:forEach varStatus="loop" var="customerOffice" items="${customerOffices}">
            marker = new google.maps.Marker({
                position : new google.maps.LatLng(${customerOffice.geoFence.latitude}, ${customerOffice.geoFence.longitude}),
                map : map
            });
        </c:forEach>
