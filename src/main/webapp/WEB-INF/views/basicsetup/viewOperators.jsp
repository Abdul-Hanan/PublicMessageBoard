                        ...
                        <div class="row align-items-center">
                            <div class="col-md-12">
                                <div class="page-header-title">
                                    <h5 class="m-b-10">Raseel Operators</h5>
                                </div>
                                <ul class="breadcrumb">
                            ...
                            <div class="col-sm-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5>Manage Operators</h5>
                                        <div class="card-header-right">
                                            <sec:authorize access="hasAuthority('create-operators')">
                                                <button type="button" onclick="location = 'operator'" class="btn btn-primary btn-icon" title="" data-toggle="tooltip" data-original-title="Create a New Operator"><i class="feather icon-plus"></i></button>
                                            </sec:authorize>
                                        </div>
                                    </div>
                                    <div class="card-block">
                                        <div class="table-responsive">
                                            <table id="key-act-button" class="display table nowrap table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                <c:forEach varStatus="loop" var="operator" items="${operators}">
                                                    <tr>
                                                        <td>${loop.index+1}</td>
                                                        <td>${operator.contactFirstName} ${operator.contactLastName}</td>
                                                        <td>${operator.contactEmail}</td>
                                                        <td>${operator.contactPhone}</td>
                                                        <td>${operator.user.username}</td>
                                                        <td>${operator.user.enabled eq true ? 'Yes':'No'}</td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('edit-operators')">
                                                                <a href="${contextPath}/operator?id=${operator.id}" class="text-left" data-cont="forms"><i class="feather feather icon-edit"></i></a>
                                                            </sec:authorize>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
