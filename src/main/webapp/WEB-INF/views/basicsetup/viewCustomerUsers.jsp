...
<head>
    <title>${customer.customerName}  Users :: Raseel</title>
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <div class="card-header-right">
                                            <sec:authorize access="hasAuthority('create-customeruser')">
                                                <button type="button" onclick="location = 'customerUser?customerId=${param.customerId}'" class="btn btn-primary btn-icon" title="" data-toggle="tooltip" data-original-title="Create a New ${customer.customerName} User"> <i class="feather icon-plus"></i></button>
                                            </sec:authorize>
                                        </div>
                                    </div>
                                    <div class="card-block">
                                        <div class="table-responsive">
                                            <table id="key-act-button" class="display table nowrap table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                <c:forEach varStatus="loop" var="customerUser" items="${customerUser}">
                                                    <tr>
                                                        <td>${loop.index+1}</td>
                                                        <td>${customerUser.contactFirstName} ${customerUser.contactMiddleName} ${customerUser.contactLastName}</td>
                                                        <td>${customerUser.contactEmail}</td>
                                                        <td>${customerUser.contactPhone}</td>
                                                        <td>${customerUser.user.username}</td>
                                                        <td>${customerUser.user.enabled eq true ? 'Yes':'No'}</td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('edit-customeruser')">
                                                                <a href="${contextPath}/customerUser?id=${customerUser.id}&customerId=${param.customerId}" class="text-left" data-cont="forms"><i class="feather feather icon-edit"></i></a>
                                                            </sec:authorize>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
