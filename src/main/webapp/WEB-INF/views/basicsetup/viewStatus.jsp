                        ...
                        <div class="row align-items-center">
                            <div class="col-md-12">
                                ...
                                <ul class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="#!">View Status</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <div class="card-header-right">
                                            <sec:authorize access="hasAuthority('create-status')">
                                                <button type="button" onclick="location = 'status'" class="btn btn-primary btn-icon" title="" data-toggle="tooltip" data-original-title="Create a New Status"><i class="feather icon-plus"></i></button>
                                            </sec:authorize>
                                        </div>
                                    </div>
                                    <div class="card-block">
                                        <div class="table-responsive">
                                            <table id="key-act-button" class="display table nowrap table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                <c:forEach varStatus="loop" var="status" items="${statuses}">
                                                    <tr>
                                                        <td>${loop.index+1}</td>
                                                        <td>${status.name}</td>
                                                        <td>${status.arabicName}</td>                                                       
                                                        <td>${status.description}</td>
                                                        <td>${status.enabled eq true ? 'Yes':'No'}</td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('edit-status')">
                                                                <a href="${contextPath}/status?id=${status.id}" class="text-left" data-cont="forms"><i class="feather feather icon-edit"></i></a>
                                                            </sec:authorize>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
