...
<head>
<title>Service Charges :: Raseel</title>
							...
							<div class="row align-items-center">
								<div class="col-md-12">
									...
									<ul class="breadcrumb">
										...
										<li class="breadcrumb-item"><a href="#!">Raseel
								...
								<div class="col-sm-12">
									<div class="card">
										<div class="card-header">
											<h5>Manage Raseel Service Charges</h5>
											<div class="card-header-right">
												<sec:authorize access="hasAuthority('create-serviceCharge')">
													<button type="button" onclick="location = 'serviceCharge'"
														class="btn btn-primary btn-icon" title=""
														data-toggle="tooltip"
														data-original-title="Create Service Charges">
										...
										<div class="card-block">
											<div class="table-responsive">
												<table id="key-act-button"
													...
													<tbody>
														<c:forEach varStatus="loop" var="serviceCharge"
															items="${serviceCharges}">
                                                    <tr style="background-color: ${serviceCharge.enabled eq false ? '#f1f1f1':'#ffffff'}">
																<td>${loop.index+1}</td>
																<td>${serviceCharge.customer.customerName}</td>
																<td>${serviceCharge.fromArea.name}</td>
																<td>${serviceCharge.toArea.name}</td>
							 									<td>${serviceCharge.shipmentType.name}</td>																
																<td>${serviceCharge.serviceType.name}</td>
																<td>${serviceCharge.serviceAmount}</td>
																<c:if test="${serviceCharge.enabled eq true}">
																<td><a href="${contextPath}/serviceChargeDisable?id=${serviceCharge.id}"
																			class="text-left" data-cont="forms">Enabled</a></td>
																</c:if>
																<c:if test="${serviceCharge.enabled eq false}">
																<td><a href="${contextPath}/serviceChargeEnable?id=${serviceCharge.id}"
																			class="text-left" data-cont="forms">Disabled</a></td>
																...
																<sec:authorize
																		access="hasAuthority('edit-serviceCharge')">
																		<a
																			href="${contextPath}/serviceCharge?id=${serviceCharge.id}"
