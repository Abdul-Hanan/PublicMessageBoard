                        ...
                        <div class="row align-items-center">
                            <div class="col-md-12">
                                <div class="page-header-title">
                                    <h5 class="m-b-10">Raseel Developer Users</h5>
                                </div>
                                <ul class="breadcrumb">
                            ...
                            <div class="col-sm-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5>Manage Developers</h5>
                                        <div class="card-header-right">
                                            <sec:authorize access="hasAuthority('create-developer')">
                                                <button type="button" onclick="location = 'developer'" class="btn btn-primary btn-icon" title="" data-toggle="tooltip" data-original-title="Create a New Developer"><i class="feather icon-plus"></i></button>
                                            </sec:authorize>
                                        </div>
                                    </div>
                                    <div class="card-block">
                                        <div class="table-responsive">
                                            <table id="key-act-button" class="display table nowrap table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                <c:forEach varStatus="loop" var="developer" items="${developers}">
                                                    <tr>
                                                        <td>${loop.index+1}</td>
                                                        <td>${developer.user.firstName} ${developer.user.lastName}</td>
                                                        <td>${developer.user.email}</td>
                                                        <td>${developer.user.username}</td>
<%--                                                         href="${contextPath}/developerApps?developerId=${developer.id}" 
 --%>                                                        <td><a class="btn btn-sm btn-rounded btn-outline-primary" data-cont="forms">
                                                                    <span class="badge badge-primary">${fn:length(developer.apps)}</span>
                                                                    Apps</a></td>
                                                        <td>${developer.user.enabled eq true ? 'Yes':'No'}</td>
                                                        <td>
                                                            <sec:authorize access="hasAuthority('edit-developer')">
                                                                <a href="${contextPath}/developer?id=${developer.id}" class="text-left" data-cont="forms"><i class="feather feather icon-edit"></i></a>
                                                            </sec:authorize>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
